...
__global__ void simulate_round(int M, int N, int T, int round, int* dHealth, int* dHealth_copy, int* dScore, int* dXC, int* dYC) {
    ...
    long long idx_source = 1LL * dYC[blockIdx.x] * M + dXC[blockIdx.x];
    long long idx_tar = 1LL * dYC[(blockIdx.x + round) % T] * M + dXC[(blockIdx.x + round) % T];
    long long idx_mid = 1LL * dYC[threadIdx.x] * M + dXC[threadIdx.x];
